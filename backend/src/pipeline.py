...
class RAGPipeline:
    ...
    def ingest(self) -> int:
        """
        Scan corpus directory, chunk documents, and build a fresh vector store snapshot.
        Atomically swaps the index pointer to prevent concurrency race conditions.
        """
        chunks = self.indexer.load_and_chunk_all()
        # Build new snapshot in isolation (Copy-On-Write)
        new_store = HybridVectorStore(index_path=self.vector_store.index_path)
    ...
    def query(
        ...
        # Grab reference to current immutable vector store snapshot
        with self._lock:
            current_store = self.vector_store

        # 1. Retrieval Stage
        retrieval_start = time.time()
        retrieved_results = current_store.search(
            query=question,
            top_k=top_k,
            threshold=threshold,
            alpha=alpha
